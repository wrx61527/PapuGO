...
from werkzeug.security import generate_password_hash, check_password_hash # BARDZO WAŻNE!
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if action == 'login':
                ...
                # !! BARDZO WAŻNE: Zahashuj hasło PRZED zapisem !!
...
def track_order_detail(order_id):
    ...
    try:
        ...
        order_items = rows_to_dicts(cursor, cursor.fetchall())
         ...
         flash('Nie masz uprawnień do tego zamówienia.', 'danger')
         return redirect(url_for('my_orders'))
    except Exception as e:
                ...
                if not name: flash('Nazwa jest wymagana.', 'warning')
                 ...
                 if not restaurant_id_str: flash('Nie podano ID.', 'warning')
                     ...
                     try:
                         ...
                         cursor.execute('DELETE FROM "Dishes" WHERE "RestaurantID" = %s', (restaurant_id,))
